    ...
    function loadControllersFrom(src, options) {
        ...
        if(src && !(src instanceof String)) {
            const name = options
                ? options.name
                : ''
            return buildMw(src, express.Router(), name)
        }
        /**
                ...
                router[info.method].call(router, path, (req, res, next) => {
                    ...
                    if(info.argsNames.includes('res')) {
                        if(ctx != undefined && ctx != null)
                            throw Error('connect-controller: You are both receiving argument res and returning a result!!! Please remove res or return undefined!!!')
